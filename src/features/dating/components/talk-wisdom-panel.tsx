...
import { addTalkWisdom, updateTalkWisdom } from "../api"
import { useTalkWisdomList } from "../hooks/useTalkWisdomList"
import type { TalkWisdomMutateValues } from "../schema"
import type { TalkWisdomVO } from "../types"
import { TalkWisdomFormDialog } from "./talk-wisdom-form-dialog"
...
export const TalkWisdomPanel = () => {
  const { list, loading, error, refetch } = useTalkWisdomList()
  const [query, setQuery] = useState("")
  const [page, setPage] = useState(1)
  const [formOpen, setFormOpen] = useState(false)
  const [formMode, setFormMode] = useState<"create" | "edit">("create")
  const [editing, setEditing] = useState<TalkWisdomVO | null>(null)
  const [detailRow, setDetailRow] = useState<TalkWisdomVO | null>(null)
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (row) =>
        ...
        (row.scene && row.scene.toLowerCase().includes(q)) ||
  ...
  const searchId = "talk-wisdom-search"
  ...
  const openEdit = (row: TalkWisdomVO) => {
    setFormMode("edit")
    setEditing(row)
    setFormOpen(true)
  }

  const openDetail = (row: TalkWisdomVO) => {
    setDetailRow(row)
  }

  const handleMutate = async (values: TalkWisdomMutateValues) => {
    ...
    if (values.id !== undefined && values.id > 0) {
      await updateTalkWisdom({ id: values.id, ...base })
    } else {
      await addTalkWisdom(base)
    }
    await refetch()
  }

  return (
    <section className="space-y-6" aria-labelledby="talk-wisdom-heading">
      <h2 id="talk-wisdom-heading" className="sr-only">
        交流智慧语录
      </h2>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-28">场景</TableHead>
          ...
          <DialogHeader className="shrink-0 space-y-2 text-left">
            ...
            {detailRow ? (
              <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 text-sm">
                <span>
                  场景：
                  {detailRow.scene?.trim() ? detailRow.scene : "—"}
                </span>
                <span>
                  标签：
                  {detailRow.tag?.trim() ? detailRow.tag : "—"}
                </span>
              </div>
            ) : null}
          </DialogHeader>
